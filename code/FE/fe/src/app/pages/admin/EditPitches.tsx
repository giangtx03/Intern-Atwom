...
import { delEditPitch, getEditPitch, getPitchTypeAll, getTimeSlotAll, postEditPitchNewVersion, putEditPitchNewVersion } from '../../service/AdminService';
...
export default function EditPitches() {
    ...
    const [timeSlotResponses, setTimeSlotResponses] = useState<TimeSlotResponse[]>([]);
    ...
    const [pitch2, setPitch2] = useState<PitchRequest>({
        ...
        address: '',
        pitchTypeId: 1,
        editPitchTimeRequestDtos: [{ price: undefined, status: '', timeSlotId: undefined }],
        imageDtos: [{ id: undefined }]
    });

    const handlePitchTimeChange = (index: number, key: keyof EditPitchTimeRequestDto, value: any) => {
    ...
    const addPitchTime = () => {
        ...
        setPitch2({
            ...pitch2,
            editPitchTimeRequestDtos: [...pitch2.editPitchTimeRequestDtos, { price: undefined, status: STATUS_PITCH_TIME_ACTIVE, timeSlotId: undefined }],
        });
    };
    ...
    const _timeSlotResponses: TimeSlotResponse[] = timeSlots
        .filter(ts => !pitch2.editPitchTimeRequestDtos.some(data => data.timeSlotId === ts.id))
        .map(ts => ({
            id: ts.id!,
    ...
    const handleDelImg = (id: number) => {
        ...
        const _idTemp: ImagePitchDto[] = updatedImages.map(data => { return { id: data.id! } });
    ...
    const savePitch = async () => {
        ...
        // Validation
        if (!pitch2.name?.trim() || !pitch2.address?.trim() || !pitch2.pitchTypeId || !pitch2.editPitchTimeRequestDtos) {
    ...
    const editPitch = (temp: EditPitchModel) => {
        ...
        const _editPitch: EditPitchTimeRequestDto[] = temp.pitchTimeChildrenDtos!.map(dto => {
            return {
                price: dto.price!,
                status: dto.status!,
        ...
        const _imgPitch: ImagePitch[] = temp.imageDtos!.map(dto => {
            return {
                id: dto.id!,
                name: dto.name!,
                pitchId: temp.id!
        ...
        const _idImg: ImagePitchDto[] = temp.imageDtos!.map(dto => {
            return { id: dto.id! }
        })
        ...
        const _timeSlotResponses = temp.pitchTimeChildrenDtos!.map(dto => {
            return {
                id: dto.idTime,
        ...
        if (foundType) {
            ...
            setPitch2({ ...pitch2, id: temp.id, name: temp.name!, address: temp.address!, pitchTypeId: foundType.id!, editPitchTimeRequestDtos: _editPitch, imageDtos: _idImg });
    ...
    const confirmDeletePitch = (temp: EditPitchModel) => {
        ...
        if (foundType) {
            setPitchType({ ...pitchType, id: foundType.id, name: foundType.name });
            setPitch2({ ...pitch2, id: temp.id, name: temp.name!, address: temp.address!, pitchTypeId: foundType.id!, editPitchTimeRequestDtos: _editPitch });
        } else {
            console.error("Pitch type not found");
        }
    ...
    return (
        <div>
            ...
            <Dialog visible={pitchDialog} style={{ width: '40rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Thông tin sân" modal className="p-fluid" footer={pitchDialogFooter} onHide={hideDialog}>
                <div className="">
                    <h3>Sân</h3>
                    <div className="field mb-3">
                        ...
                        <InputText id="name" value={pitch2.name} onChange={(e) => onInputChange(e, 'name')} required autoFocus className={classNames({ 'p-invalid': submitted && !pitch2.name })} />
                        {submitted && !pitch2.name && <small className="p-error">Tên sân không được để trống.</small>}
                    </div>
                    <div className="field mb-3">
                        <label htmlFor="address" className="font-bold">
                            Địa chỉ
                        </label>
                        <InputText id="address" value={pitch2.address} onChange={(e) => onInputChange(e, 'address')} required autoFocus className={classNames({ 'p-invalid': submitted && !pitch2.address })} />
                        {submitted && !pitch2.address && <small className="p-error">Địa chỉ không được để trống.</small>}
                    </div>

                    <div className="field">
                        <label htmlFor="pitchTypeId">Loại sân</label>
                        <Dropdown value={pitchTypes.find(e => e.id === pitch2.pitchTypeId)} onChange={(e) => onDropdownChange(e, 'pitchTypeId')} options={pitchTypes} optionLabel="name" placeholder="Chọn loại sân" className={classNames({ 'p-invalid': submitted && !pitch2.pitchTypeId })} />
                        {submitted && !pitch2.pitchTypeId && <small className="p-error">Vui lòng chọn loại sân.</small>}
                    </div>
                </div>
                <hr />
                <div className="mt-3">
                        ...
                        <div className='mt-3' key={index}>
                            <div className="d-flex gap-2">
                                <Dropdown
                                    ...
                                    className={submitted && !timeSlotResponses[index]?.id ? 'p-invalid' : ''}
                                ...
                                <InputText
                                    ...
                                    onChange={(e) => handlePitchTimeChange(index, 'status', e.target.value)}
                                    hidden
                                />
                                <div className="">
                                    <Button icon="pi pi-trash" rounded outlined severity="danger" onClick={() => removePitchTime(index)} />
                                </div>
                            </div>
                        </div>
                    ))}
                    <div className="mt-3">
                        {timeSlots.length > (countIndex) && <Button label="Thêm khung giờ" icon="pi pi-plus" severity="success" onClick={addPitchTime} />
                ...
                <div className="mt-3">
                    ...
                    <div className="my-2">
                        {previewUrls.map((url, index) => (
                            <div className="m-2" style={{ position: 'relative', display: 'inline-block', width: 200 }}>
                                <img
                                    ...
                                    src={url}
                                    alt={`Preview ${index + 1}`}
                                    style={{ width: 200, height: 200, objectFit: 'cover', border: "1px solid " }}
                                />
                                <span
                                    style={{
                                        position: 'absolute',
                        ...
                        {imgPitches.map(pitchImg => (
                            <div key={pitchImg.id} className="m-2" style={{ position: 'relative', display: 'inline-block', width: 200 }}>
                                <img
